...
func main() {
	...
	dataTree.CreateNode("/root/node1", []byte{1, 2, 3, 4}, 1, time.Now().Unix(), 0)
	...
	fmt.Printf("Root node added has data: %d\n", dataTree.GetData("/root/node1"))
	dataTree.SetData("/root/node1", []byte{2, 3, 4, 5}, 2, 1, time.Now().Unix())
	fmt.Printf("Root node added has new data: %d\n", dataTree.GetData("/root/node1"))

	// Delete Node
	dataTree.DeleteNode("/root/node1", 0)
